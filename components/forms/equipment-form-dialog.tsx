            ...
            <TabsContent value="detalles" className="space-y-4 pt-4 outline-none">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <AsyncCombobox value={keyWinId} onValueChange={setKeyWinId} fetcher={fetchWinLic} placeholder="Selecciona licencia Windows" preloadItems={[{id: "_null", nombre: "Sin Licencia Windows", key: ""}, {id: "clie", nombre: "Licencia de marca", key: ""}, ...filteredWinLicenses]} filterItem={(item) => item.id === "_null" || item.id === "clie" || item.id === "marca" || (item.activa !== false && (item.usos === undefined || Number(item.usos) > 0)) || String(item.id) === String(keyWinId)} renderItem={(item) => `${item.nombre} ${item.key ? "- " + item.key : ""} ${item.activa === false ? "(Inactiva)" : ""}`} renderValue={(item) => `${item.nombre} ${item.key ? "- " + item.key : ""}`} />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="eq-key-office">Licencia Office</Label>
                  <AsyncCombobox value={keyOfficeId} onValueChange={setKeyOfficeId} fetcher={fetchOfficeLic} placeholder="Selecciona licencia Office" preloadItems={[{id: "_null", nombre: "Sin Licencia Office", key: ""}, {id: "marca", nombre: "Cliente ya posee una", key: ""}, ...filteredOfficeLicenses]} filterItem={(item) => item.id === "_null" || item.id === "clie" || item.id === "marca" || (item.activa !== false && (item.usos === undefined || Number(item.usos) > 0)) || String(item.id) === String(keyOfficeId)} renderItem={(item) => `${item.nombre} ${item.key ? "- " + item.key : ""} ${item.activa === false ? "(Inactiva)" : ""}`} renderValue={(item) => `${item.nombre} ${item.key ? "- " + item.key : ""}`} />
                </div>
              </div>
