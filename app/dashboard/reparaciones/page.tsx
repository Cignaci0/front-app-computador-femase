...
import { ReparacionFormDialog } from "@/components/forms/reparacion-form-dialog"
...
export default function ReparacionesPage() {
  ...
  const [reparacionToEdit, setReparacionToEdit] = useState<any | null>(null)
  ...
  const fetchData = async (p: number, s: string) => {
    ...
    try {
      if (s && s.trim() !== "") {
        const items = await searchReparaciones(s)
        setData(items || [])
        setMeta({ total: items?.length || 0, page: 1, lastPage: 1, limit: items?.length || 10 })
      } else {
        const response = await getReparaciones(p, 10)
      ...
      console.error(error)
      toast.error("Error al obtener reparaciones")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSave = async (formData: any) => {
    try {
      if (reparacionToEdit) {
        await updateReparacion(reparacionToEdit.id, formData)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-balance">Reparaciones</h1>
          <p className="text-muted-foreground">
            Gestión de reparaciones de computadores y equipos
          </p>
        </div>
        <Button onClick={() => {
          setReparacionToEdit(null)
          setDialogOpen(true)
        }}>
      ...
      <Card className="bg-card border-border">
        <CardHeader className="pb-3 flex flex-row items-center justify-between">
          <CardTitle className="text-base font-medium">
            {isLoading ? "Cargando..." : `${data.length} reparaciones encontradas`}
          </CardTitle>
        </CardHeader>
                    ...
                    <TableRow key={item.id} className="border-border hover:bg-secondary/10">
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          {item.upgrade && (
                            <Badge variant="secondary" className="font-normal bg-green-500/10 text-green-500">
                              Upgrade
                            </Badge>
                          )}
                          {item.mantencion && (
                ...
                {!isLoading && data.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                      No se encontraron registros de reparación.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
      ...
      <ReparacionFormDialog
        open={dialogOpen}
        onOpenChange={setDialogOpen}
        reparacionToEdit={reparacionToEdit}
        onSave={handleSave}
      />
    </div>
  )
